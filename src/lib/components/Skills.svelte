...
<div class="flex items-center justify-center h-full px-4 sm:px-6 md:px-8">
  <div class="max-w-6xl mx-auto w-full">
    <div class="font-mono tracking-[0.3em] uppercase text-[#6366f1] mb-2" style="font-size: clamp(0.6rem, 0.7vw, 0.75rem);">03 // Skills</div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
      {#each skillCategories as category, i}
        <ScrollReveal delay={i * 60}>
          ...
          >
            <h3 class="font-mono font-semibold tracking-wider text-[#f1f5f9] mb-3 uppercase" style="font-size: clamp(0.55rem, 0.65vw, 0.7rem);">
              {category.label}
            </h3>
